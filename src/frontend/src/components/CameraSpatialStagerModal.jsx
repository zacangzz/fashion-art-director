...
/**
 * Accessible Modal dialog for the Interactive 2D/3D Camera & Subject Spatial Stager.
 * Serves as the complete 3D scene studio for perspective, subject positioning,
 * camera vectors, lens focal length, optical depth of field, and photometric lighting.
 */
export default function CameraSpatialStagerModal({
  isOpen,
  onClose,
  backgroundImageUrl,
  stagingParams = {},
  ...
  const handlePointerMove = useCallback(
    (e) => {
      ...
      const clampedY = Math.max(0.06, Math.min(0.94, rawY));
  ...
  const handleResetDefaults = () => {
    ...
    onDepthOfFieldChange?.('natural');
    onLightingModeChange?.('harmonize_ambient');
  };
  ...
  const modalFooter = (
    <div className="stager-modal-footer">
      <Button
        variant="ghost"
        size="sm"
  ...
  return (
    ...
    >
      <div className="camera-spatial-stager modal-mode" role="region" aria-label="3D Camera & Subject Spatial Stager">
        ...
        {/* Interactive Staging Viewport (Enlarged inside Modal) */}
        <div
          ref={containerRef}
          className="stager-canvas-wrap modal-stage-canvas"
          tabIndex={0}
        ...
        >
          ...
          <svg className="stager-fov-svg" aria-hidden="true">
            <defs>
              <radialGradient id="fovGradModal" cx="0%" cy="0%" r="100%">
          ...
          >
            ...
            <div className="pin-body subject-body modal-pin-body">
              <User size={15} className="pin-icon" />
            </div>
            <span className="pin-label">Subject ({getSubjectPositionText()})</span>
          </div>
          ...
          >
            ...
            <div
              className="pin-body camera-body modal-pin-body"
              style={{ transform: `rotate(${pointingAngleDeg}deg)` }}
            >
              <Camera size={15} className="pin-icon" />
        ...
        <div className="stager-lens-section">
          <div className="stager-slider-header">
            <span className="stager-section-title">
              ...
              Lens Focal Length & Zoom Framing:
            </span>
          ...
          <div className="stager-lens-scale">
            ...
            <span>35mm (Environmental)</span>
            <span>50mm (Standard Eye-Level)</span>
            <span>85mm (Portrait Crop)</span>
          </div>
        </div>

        {/* Perspective & Camera Alignment Presets */}
        ...
        <div className="stager-angle-presets">
          ...
          <div className="stager-chip-group">
            {[
              { id: 'natural', label: '📷 Natural Focus' },
              { id: 'cinematic_bokeh', label: '✨ Cinematic Bokeh (f/1.4)' },
              { id: 'crisp_architectural', label: '🏢 Crisp Architectural (f/8)' },
            ].map((item) => (
              <button
                key={item.id}
                type="button"
                className={`stager-preset-chip ${depthOfField === item.id ? 'active' : ''}`}
                onClick={() => onDepthOfFieldChange?.(item.id)}
              >
                {item.label}
              </button>
            ))}
        ...
        {/* Photometric Lighting Harmonization */}
        <div className="stager-angle-presets">
          <span className="stager-section-title">
          ...
          <div className="stager-chip-group">
            {[
              { id: 'harmonize_ambient', label: '☀️ Ambient Spill & Shadows' },
              { id: 'match_white_balance', label: '🎨 Calibrated White Balance' },
              { id: 'dramatic_contrast', label: '🌓 Dramatic Contrast' },
            ].map((item) => (
              <button
                key={item.id}
                type="button"
                className={`stager-preset-chip ${lightingMode === item.id ? 'active' : ''}`}
                onClick={() => onLightingModeChange?.(item.id)}
              >
                {item.label}
              </button>
            ))}
        ...
        <div className="stager-summary-pill" role="status" aria-live="polite">
          ...
          <span>
            <strong>Active 3D Staging:</strong> {focalLength}mm {zoomLevel} • {getCameraVantageText()} • Subject at {getSubjectPositionText()} • {depthOfField.replace('_', ' ')} • {lightingMode.replace('_', ' ')}
          </span>
        </div>
      </div>
    </Modal>
